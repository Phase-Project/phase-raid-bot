...
from core.config import OWNER_IDS, OWNER_BLACKLIST_BYPASS, MAIN_SERVER_ID, VERIFIED_ROLE_ID
from core.utils.db import is_server_blacklisted, is_user_blacklisted
from core.config import WARN_EMOJI, DENY_EMOJI
...
async def global_interaction_check(interaction: discord.Interaction) -> bool:
    ...
    if await is_user_blacklisted(str(user_id)):
        await interaction.response.deny(f"{DENY_EMOJI} You are blacklisted from using the bot.", ephemeral=True)
        return False

    guild_id = interaction.guild_id

    if guild_id and await is_server_blacklisted(str(guild_id)):
        await interaction.response.deny(f"{DENY_EMOJI} This server is blacklisted.", ephemeral=True)
        return False
    ...
    if MAIN_SERVER_ID and guild_id is not None and int(guild_id) == MAIN_SERVER_ID:
        await interaction.response.deny(f"u can't raid this server lil bro {WARN_EMOJI}", ephemeral=True)
        return False
